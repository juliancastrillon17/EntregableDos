...
function tienenMismaLongitud(str1, str2) {
  ...
  const pasarUsuarioAPremium = usuarios => {
    ...
    for(let element in usuarios){
      usuarios[element].esPremium = true;
    }
  ...
  const sumarLikesDeUsuario = usuario => {
    ...
    // };
      ...
      for(let element in usuario.posts){
        suma += usuario.posts[element].likes;
      }
...
const crearInstanciaPersona = (nombre, apellido, edad, dir)=>{
    ...
    //tu código: 
    let personaDos = new Persona(nombre , apellido, edad, dir);
    return personaDos;
}
...
const sumarArray = (numeros, cb) =>{
    ...
    for(let element in numeros){
       suma += numeros[element];
     }
